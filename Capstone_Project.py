...
### START: TRÁNH LÀM CHẬM HỆ THỐNG DO PHẢI XỬ LÝ LẠI MỖI KHI CHỌN CÁC CHỨC NĂNG ###
    ...
    if 'df_report' not in st.session_state:
        ...
        temp = rfm_df.reset_index(drop=True)
        df_report  = pd.concat([df_report , temp], axis = 1).loc[:,["order_dt", "order_products", "order_amount", "month", "RFM_Score", "Customer_Group"]].reset_index()
